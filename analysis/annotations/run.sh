...
# # print workflow
snakemake -nprs ${snakedir}/Snakefile --rulegraph --configfile ${configdir}/config.yaml | dot -Tsvg > ${configdir}/${workflow}.svg
...
snakemake --cluster-config ${configdir}/cluster.yaml \
		  ...
		  -j 60 --latency-wait 400 -prs ${snakedir}/Snakefile --configfile ${configdir}/config.yaml $1
