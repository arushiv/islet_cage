...
def makeResultDf(all_files, string, split, infosplit):
    dlist = []
    for f in all_files:
        d = pandas.read_csv(f, comment="#", sep="\t", usecols=['observed','expected', 'CI95low', 'CI95high', 'fold', 'l2fold', 'stddev', 'qvalue', 'pvalue'])
        d['feature'] = os.path.basename(f).replace(string, "")
        dlist.append(d)
        
    df  = pandas.concat(dlist, ignore_index=True)
    print(df.head())
    if split is not None:
        df[infosplit] = df['feature'].str.split(split, expand=True)
