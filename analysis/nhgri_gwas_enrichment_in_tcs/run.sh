...
# # print workflow
snakemake -nprs ${snakedir}/Snakefile_prune --rulegraph --configfile ${configdir}/config.yaml | dot -Tsvg > ${configdir}/${workflow}_prune.svg
...
# 		  -j 60 --latency-wait 400 -prs ${snakedir}/Snakefile_prune --configfile ${configdir}/config.yaml $1
...
# Run GWAS enrichment
snakemake -nprs ${snakedir}/Snakefile_GREGOR --rulegraph --configfile ${configdir}/config_gregor.yaml | dot -Tsvg > ${configdir}/${workflow}_gregor.svg

# Submit jobs
snakemake --cluster-config ${configdir}/cluster.yaml \
		  --cluster "sbatch --time {cluster.time} --mem {cluster.mem} --cpus-per-task {cluster.cpus} --job-name {cluster.jobname} -o {cluster.output} -e {cluster.error}  --parsable "  \
		  -j 60 --latency-wait 400 -prs ${snakedir}/Snakefile_GREGOR --configfile ${configdir}/config_gregor.yaml $1
