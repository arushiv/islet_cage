...
# # print workflow
snakemake -nprs ${snakedir}/Snakefile_atac_seq --rulegraph --configfile ${configdir}/config.yaml | dot -Tsvg > ${configdir}/${workflow}.svg
...
snakemake --cluster-config ${configdir}/cluster.yaml --use-conda \
		  ...
		  -j 60 --latency-wait 400 -prs ${snakedir}/Snakefile_atac_seq --configfile ${configdir}/config.yaml $1
