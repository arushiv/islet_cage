...
# # print workflow
snakemake -nprs ${scriptdir}/Snakefile_atac_seq --rulegraph --configfile ${configdir}/config.yaml | dot -Tsvg > ${configdir}/${workflow}.svg

# Submit jobs
snakemake --cluster-config ${configdir}/cluster.yaml --use-conda \
		  ...
		  -j 60 --latency-wait 400 -prs ${scriptdir}/Snakefile_atac_seq --configfile ${configdir}/config.yaml $1
